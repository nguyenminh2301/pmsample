...
アプリは階層的に構成されています（カテゴリ A: 二値アウトカム）。

### A. 二値アウトカム (Binary Outcomes)

#### A1. クイックチェック (Quick Checks)

| 方法 | 説明 | 目的 |
| :--- | :--- | :--- |
| **A1.1: イベント対変数比 (EPV)** | 経験則（ヒューリスティック）。 | 実現可能性の確認。 |
| **A1.2: ベースラインリスクの精度** | 特定の CI 幅で有病率を推定。 | 記述疫学。 |

#### A2. 予後因子 (Prognostic Factors)

| 方法 | 説明 | 目的 |
| :--- | :--- | :--- |
| **A2.1: ロジスティック検出力 (Hsieh)** | オッズ比 (OR) を検出する検出力。 | 関連性研究。 |
| **A2.2: Cox 検出力 (Schoenfeld)** | ハザード比 (HR) を検出する検出力。 | 生存分析。 |

#### A3. モデル開発 (Prediction Model Development)

| 方法 | 説明 | 目的 |
| :--- | :--- | :--- |
| **A3.1: 解析的アプローチ (Riley)** | **推奨。** シュリンク、楽観、精度を調整。 | 新規モデル開発。 |
| **A3.2: シミュレーション設計** | 複雑なシナリオのための DGM シミュレーション。 | 開発（複雑）。 |
| **A3.3: ベイズ保証 (Assurance)** | 成功確率を保証する MCMC シミュレーション。 | ベイズモデル開発。 |

#### A4. 検証と更新 (Validation / Updating)

| 方法 | 説明 | 目的 |
| :--- | :--- | :--- |
| **A4.1: AUC 精度** | AUC (C統計量) の CI 幅。 | 検証（識別能）。 |
| **A4.2: 外部検証** | O/E、キャリブレーションスロープ、AUC の精度。 | 検証（完全）。 |
| **A4.3: 検証シミュレーション** | LP 分布に基づくシミュレーション。 | 検証（シミュレーション）。 |
| **A4.4: モデル更新** | 再キャリブレーション（切片/傾き）のためのサンプルサイズ。 | モデル更新。 |
