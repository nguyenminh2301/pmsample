...
애플리케이션은 계층적으로 구성됩니다 (카테고리 A: 이진 결과).

### A. 이진 결과 (Binary Outcomes)

#### A1. 빠른 확인 (Quick Checks)

| 방법 | 설명 | 목표 |
| :--- | :--- | :--- |
| **A1.1: 변수 당 이벤트 (EPV)** | 경험적 규칙 (휴리스틱). | 타당성 확인. |
| **A1.2: 기본 위험의 정밀도** | 지정된 CI 폭으로 유병률 추정. | 기술 역학. |

#### A2. 예후 인자 (Prognostic Factors)

| 방법 | 설명 | 목표 |
| :--- | :--- | :--- |
| **A2.1: 로지스틱 검정력 (Hsieh)** | 목표 오즈비(OR)를 탐지하는 검정력. | 연관성 연구. |
| **A2.2: Cox 검정력 (Schoenfeld)** | 목표 위험비(HR)를 탐지하는 검정력. | 생존 분석. |

#### A3. 모델 개발 (Prediction Model Development)

| 방법 | 설명 | 목표 |
| :--- | :--- | :--- |
| **A3.1: 분석적 접근 (Riley)** | **권장.** 수축, 낙관주의, 정밀도 조정. | 새로운 모델 개발. |
| **A3.2: 시뮬레이션 기반 설계** | 복잡한 시나리오를 위한 DGM 시뮬레이션. | 개발 (복잡함). |
| **A3.3: 베이지안 보증** | 성공 확률이 보장된 MCMC 시뮬레이션. | 베이지안 모델 개발. |

#### A4. 검증 및 업데이트 (Validation / Updating)

| 방법 | 설명 | 목표 |
| :--- | :--- | :--- |
| **A4.1: AUC 정밀도** | AUC(C-통계량)에 대한 CI 너비. | 검증 (판별력). |
| **A4.2: 외부 검증** | O/E, 교정 기울기, AUC의 정밀도. | 검증 (전체). |
| **A4.3: 검증 시뮬레이션** | LP 분포를 기반으로 한 시뮬레이션. | 검증 (시뮬레이션). |
| **A4.4: 모델 업데이트** | 재교정(절편/기울기)을 위한 표본 크기. | 모델 업데이트. |
